...
//            DefaultAmbientDisplay, StockAmbientDisplay, WeatherAmbientDisplay, CurrentCityWeatherAmbientInfo, 
//            WeatherAmbientInfo, CityClockImageRepo, StockAmbientInfo, IAmbientDisplay, 
//            IAmbientInfo

public class WriteAmbientInfo
{
    ...
    private static final String TAG = "WriteAmbientInfo";
    private static final int WHITE_IMAGE = 2;
    private IAmbientDisplay ambientDisplay;

    public WriteAmbientInfo(IAmbientDisplay iambientdisplay)
    {
        ambientDisplay = iambientdisplay;
    }
    ...
    public IAmbientDisplay createAmbientImageFromInfo()
        throws Exception
    {
label0:
        {
...
label3:
                    {
                        ...
                        if (ambientDisplay instanceof StockAmbientDisplay)
                        {
                            ...
                            IAmbientInfo iambientinfo1 = stockambientdisplay.getAmbientInfo();
                            Bitmap bitmap4 = stockambientdisplay.writeClockDataOntoBitmap(iambientinfo1, 2);
                            if (bitmap4 != null)
                            {
                                ...
                                if (file7 != null)
                                {
                                    writeFileForSourceLocation(defaultambientdisplay, abyte2, file7, iambientinfo1, 4);
                                    Bitmap bitmap5 = stockambientdisplay.writeClockDataOntoBitmap(iambientinfo1, 0);
                                    Log.d("WriteAmbientInfo", "Clock Image is created successfully");
                                    if (bitmap5 != null)
                                    {
                                        ...
                                        if (file8 != null)
                                        {
                                            writeFileForSourceLocation(defaultambientdisplay, abyte3, file8, iambientinfo1, 5);
                                            Bitmap bitmap6 = stockambientdisplay.writeDataOntoBitmap(iambientinfo1);
                                            Log.d("WriteAmbientInfo", "Clock Image is created successfully");
                                            if (bitmap6 != null)
                                            {
                                                ...
                                                if (file9 != null)
                                                {
                                                    writeFileForSourceLocation(defaultambientdisplay, abyte4, file9, iambientinfo1, 6);
                                                } else
                                                {
                                                    Log.d("WriteAmbientInfo", "Can not write File on Sd Card ,Media not mounted");
                                                    return null;
                                                }
                                            } else
                                            {
                                                Log.e("WriteAmbientInfo", "Returning Bitmap is null File not written");
                                                return null;
                                            }
                                        } else
                                        {
                        ...
                        if (ambientDisplay instanceof WeatherAmbientDisplay)
                        {
                            ...
                            Log.d("Timer", (new StringBuilder()).append("timer start").append(new Date(l)).toString());
                            WeatherAmbientDisplay weatherambientdisplay = (WeatherAmbientDisplay)ambientDisplay;
                            IAmbientInfo iambientinfo = weatherambientdisplay.getAmbientInfo();
                            if (iambientinfo instanceof CurrentCityWeatherAmbientInfo)
                                ...
                                {
                                    Log.d("WriteAmbientInfo", "Processing CurrentCityAmbientInfo Images");
                                    ...
                                    {
                                        Log.d("WriteAmbientInfo", "Repo got created for CurrentCity Ambient Info");
                                        for (int i = 0; i < 3; i++)
                                        {
                                        ...
                                        if (file4 != null)
                                        {
                                            ...
                                            if (file5.exists())
                                            {
                                                ...
                                                Log.d("WriteAmbientInfo", (new StringBuilder()).append("ZIp Destination Location : ").append(file6.getAbsolutePath()).toString());
                                                ArrayList arraylist = new ArrayList();
                                                generateSourceFileList(file5, file5.getAbsolutePath(), arraylist);
                                                Log.d("WriteAmbientInfo", (new StringBuilder()).append("List of files").append(arraylist).toString());
    ...
    public void createBitmapSourceLocation(DefaultAmbientDisplay defaultambientdisplay, Bitmap bitmap, Bitmap bitmap1, IAmbientInfo iambientinfo, int i)
    {
        ...
        if (file != null)
        {
            ...
            if (!file1.exists() && !file1.mkdirs())
                Log.e("WriteAmbientInfo", "Error Creating currentCityWeatherPath folder on SD Card");
            ...
            Log.d("WriteAmbientInfo", (new StringBuilder()).append("File written ").append(file2.getPath()).append("Weath Cond file").append(file3.getPath()).toString());
            return;
        } else
        {
            Log.d("WriteAmbientInfo", "parent directory returned as null");
            return;
        }
    }
...
_L1:
        ...
        Log.d("WriteAmbientInfo", (new StringBuilder()).append("File Added : ").append(s2).toString());
...
_L4:
        ...
        zipoutputstream1.closeEntry();
          goto _L6
_L2:
        Log.d("WriteAmbientInfo", "Zipping Done");
...
_L7:
    ...
    public void generateSourceFileList(File file, String s, List list)
    {
        ...
        if (file.isDirectory())
        {
            Log.d("WriteAmbientInfo", "Insise a directory");
            String as[] = file.list();
            int i = as.length;
            for (int j = 0; j < i; j++)
    ...
    public void writeFileForSourceLocation(DefaultAmbientDisplay defaultambientdisplay, byte abyte0[], File file, IAmbientInfo iambientinfo, int i)
    {
        if (defaultambientdisplay instanceof StockAmbientDisplay)
        {
            ...
            if (file2 != null)
                Log.d("WriteAmbientInfo", (new StringBuilder()).append("File written ").append(file2.getPath()).toString());
            Log.d("WriteAmbientInfo", (new StringBuilder()).append("Android Path ").append(stockambientinfo.getSourceLocation()).toString());
            Log.d("WriteAmbientInfo", (new StringBuilder()).append("WD Path ").append(stockambientinfo.getDestinationLocation()).toString());
        } else
        if (defaultambientdisplay instanceof WeatherAmbientDisplay)
        {
            ...
            weatherambientdisplay.setAmbientInfo(weatherambientinfo);
            Log.d("WriteAmbientInfo", (new StringBuilder()).append("File written ").append(file1.getPath()).toString());
            Log.d("WriteAmbientInfo", (new StringBuilder()).append("Android Path ").append(weatherambientinfo.getSourceLocation()).toString());
            Log.d("WriteAmbientInfo", (new StringBuilder()).append("WD Path ").append(weatherambientinfo.getDestinationLocation()).toString());
            return;
        }
    }
}
